...
import {hideLoader, showLoader, updateProduct} from '../../redux/slices';
import {
  COLORS,
  CONSTANTS,
  FONTS,
...
export default function EditProduct(props) {
  ...
  const [brand, setBrand] = useState(
    brands.find(i => i.title === product?.brandId),
  );
  ...
  const onUpdateProduct = async () => {
    ...
    for (let i = 0; i < productImages.length; i++) {
      ...
      if (item.startsWith('/uploads/products/img')) {
        const img = await utils.handleBase64(
          CONSTANTS.API_URLS.IMAGE_URL + item,
        );
        images.push(img);
      } else {
        images.push(item);
  ...
  return (
    <SafeAreaView style={STYLES.container}>
      <CustomHeader showBackButton title="Edit Product" />
            ...
            <View style={styles.photosView}>
              {productImages.map((item, index) => (
                <View
                  ...
                  <Image
                    source={{
                      uri: item.startsWith('/uploads')
                        ? CONSTANTS.API_URLS.IMAGE_URL + item
                        : item,
                    }}
                    style={styles.dummyImgStyle}
                  />
